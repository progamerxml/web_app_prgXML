        ...
        <div class="container-fluid">
            <table class="table table-bordered table-stripped">
                ...
                <tbody>
                    <?php 
					$i = 1;
						$qry = $conn->query("SELECT e.*, CONCAT(e.last_name, ', ' , e.first_name,' ', COALESCE(e.middle_name,'')) as fullname, d.name as department, p.name as position from `employee_list` e inner join department_list d on e.department_id = d.id inner join position_list p on e.position_id = p.id where e.delete_flag = 0 order by `fullname` asc ");
